...
const getOrderList = (page, callback) => {
  var para = {
    member_id: util.storageMethod.get('member_id'),
    member_order_id: '1',
    pagination: {
      page: page,
      count: getApp().globalData.pageCount
    }
  }
  console.log('globalData => ', getApp().globalData.pageCount);
  BaseHttp.post(API.getWareHouseProductList , para, (d, status) => {
    if (status) callback(d);
    else util.showModalWithNotice('提示', '请求失败:' + JSON.stringify(d));
  });
}
